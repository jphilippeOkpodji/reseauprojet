...
/*
 * fait la même chose que rechercheSocket mais retourne l'ancien pseudo .
...
/*
* Cette fonction sert à ajouter un pseudo
*
*/
...
/*
 * va mettre la liste des joueurs connectés dans la donnee qui sera ensuite envoyé à celui qui a demandé la liste des joueurs
 */

char* message_list(Liste_Clients* liste) {
    char* donnee = "liste des personnes connectées :\n";
    Case* p = liste->beginp;
    
    while (p != NULL) {
        donnee = conception_message(donnee, p->pseudo);
        donnee = conception_message(donnee, ", ");
        p = p->nextp;
    }
...
char* menu(){
    ...
    data = conception_message(data,"\n");
    char *data4 = "* /quitter pour quitter le chat                                                                *" ;
